...
if ( ! function_exists( 'onepress_footer_site_info' ) ) {
    ...
    {
        ?>
        <?php printf(esc_html__('Alguns Direitos Reservados %1$s %2$s %3$s', 'onepress'), ' - Ilha Grande/RJ', esc_attr(date('Y')), esc_attr(get_bloginfo())); ?>
        <span class="sep"> &ndash; </span>
        <?php printf(esc_html__('OnePress Child theme by %1$s', 'onepress'), '<a href="' . esc_url('https://brasa.art.br', 'onepress') . '">Brasa</a>'); ?>
        <?php
    }
}
